...
RExpr transitive(RExpr e) = implies(compose([e, e]), e);
RExpr reflexive(RExpr e) = implies(union({idDom(e), idRan(e)}), e);
RExpr symmetric(RExpr e) = implies(inv(e), e);
//RExpr univalent(RExpr e) = implies(compose([inv(e), e]), idRan(e));
RExpr total(RExpr e) = implies(idDom(e), compose([e, inv(e)]));
//RExpr surjective(RExpr e) = implies(idRan(e), compose([inv(e), e]));

RExpr base(Id relation, EId from, EId to) = base("<relation>", "<from>", "<to>");
RExpr id(EId c) = id("<c>");

data Rule
  = rule(loc origin, RExpr expr)
  ;
...
list[Rule] toRules((Decl)`rule <Id name>: <Invariant invariant>`, EId owner, Env env)
  = [rule(name@\loc, invariant2rexpr(invariant, owner, env))]; 
...
RExpr mod2rexpr(m:(Modifier)`ref`, Id name, EId owner, EId target)
  = reflexive(base(name, owner, target));
  
RExpr mod2rexpr(m:(Modifier)`trans`, Id name, EId owner, EId target)
  = transitive(base(name, owner, target));
...
RExpr mod2rexpr(m:(Modifier)`sym`, Id name, EId owner, EId target)
  = symmetric(base(name, owner, target));
